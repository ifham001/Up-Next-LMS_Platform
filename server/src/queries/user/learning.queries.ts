  ...
  try {
    const userCoursesList = await dbDrizzle
      .select({
        ...
        userCourseId: userCourses.id, // alias to match frontend
      })
      .from(userCourses)
    ...
    const courseDetailWithUserProgress = await Promise.all(
      userCoursesList.map(async (c) => {
        const getProgress = await courseProgress(c.userCourseId);
        const progress = getProgress?.completedPercent ?? 0;
...
export const getCourseContentById = async (userId: string, courseId: string) => {
  try {
    ...
    return {
      ...
      sections,
      userCourseId: user_courses[0].id, // ✅ return singular for frontend
    };
  } catch (error) {
    console.error(error);
...
// ✅ Get quiz/resource/video item by ID
